...
pub enum StorageEngine {
    ...
    Memory,
    /// Local file-based storage (legacy)
    LocalKv,
    /// SurrealKV storage (recommended for local apps)
    SurrealKv,
    /// TiKV distributed storage (for clustered deployments)
    TiKv,
    /// WebSocket connection to a remote SurrealDB server
    WebSocket,
}
...
impl DbConfig {
    ...
    pub fn with_migrations(mut self, run_migrations: bool) -> Self {
