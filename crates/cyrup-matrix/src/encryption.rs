    ...
    pub fn accept_sas(&self) -> MatrixFuture<CyrumSasVerification> {
        ...
        MatrixFuture::spawn(async move {
            match inner {
                ...
                _ => Err(EncryptionError::InvalidVerificationType(
...
impl CyrumEncryption {
    ...
    pub fn verify_device(&self, user_id: &UserId, device_id: &DeviceId) -> MatrixFuture<CyrumVerificationRequest> {
        let user_id = user_id.to_owned();
        let device_id = device_id.to_owned();
        let client = self.client.clone();
        
        MatrixFuture::spawn(async move {
            let request = client.encryption().get_verification(&user_id, Some(&device_id)).await
                .map_err(EncryptionError::matrix_sdk)?;
    ...
    pub fn verify_user(&self, user_id: &UserId) -> MatrixFuture<CyrumVerificationRequest> {
        ...
        MatrixFuture::spawn(async move {
            let request = client.encryption().get_verification(&user_id, None).await
                .map_err(EncryptionError::matrix_sdk)?;
    ...
    pub fn enable_backup(&self) -> MatrixFuture<String> {
        ...
        MatrixFuture::spawn(async move {
            ...
            let version = client.encryption().create_backup_version().await
                .map_err(EncryptionError::matrix_sdk)?;
                
            // Then we need to enable the backup with that version
            client.encryption().enable_backup_v1(version.clone()).await
                .map_err(EncryptionError::matrix_sdk)?;
    ...
    pub fn has_backup(&self) -> MatrixFuture<bool> {
        ...
        MatrixFuture::spawn(async move {
            let info = client.encryption().backup_info().await
                .map_err(EncryptionError::matrix_sdk)?;
    ...
    pub fn export_recovery_key(&self) -> MatrixFuture<String> {
        ...
        MatrixFuture::spawn(async move {
            ...
            let backup_info = client.encryption().backup_info().await
                .map_err(EncryptionError::matrix_sdk)?;
            ...
            let recovery_key = client.encryption().export_backup_key(backup_info.version).await
                .map_err(EncryptionError::matrix_sdk)?;
    ...
    pub fn import_recovery_key(&self, recovery_key: &str) -> MatrixFuture<usize> {
        ...
        MatrixFuture::spawn(async move {
            ...
            let result = client.encryption().receive_room_keys_from_backup(
                &version,
                &key,
    ...
    pub fn get_device_verification(&self, user_id: &UserId, device_id: &DeviceId) -> MatrixFuture<VerificationState> {
        let user_id = user_id.to_owned();
        let device_id = device_id.to_owned();
        let client = self.client.clone();
        
        MatrixFuture::spawn(async move {
            let device = client.encryption().get_device(&user_id, &device_id).await
                .map_err(EncryptionError::matrix_sdk)?;
    ...
    pub fn subscribe_to_verification_requests(&self) -> MatrixStream<CyrumVerificationRequest> {
        ...
        MatrixStream::spawn(async move {
            ...
            client.add_event_handler(move |ev: matrix_sdk::encryption::verification::VerificationRequest| {
                let sender = sender.clone();
                
                async move {
                    match ev.accept().await {
                        Ok(verification) => {
                            let _ = sender.send(Ok(CyrumVerificationRequest::new(verification))).await;
                        }
                        Err(e) => {
