...
impl matrix_sdk_base::store::StateStore for SurrealStateStore {
    ...
    #[instrument(skip(self, request_id), level = "debug")]
    fn mark_media_upload_started(&self, request_id: &str) -> MediaUploadFuture<()> {
        let request_id = request_id.to_owned();
        let client = self.client.clone();

        MediaUploadFuture::new(async move {
            let media_upload_dao = crate::db::MediaUploadDao::new(client);

            debug!(request_id = %request_id, "Marking media upload as started");

            match media_upload_dao.mark_upload_started(&request_id).await {
                Ok(_) => {
                    debug!(request_id = %request_id, "Media upload marked as started");
                    Ok(())
                },
                Err(err) => {
    ...
    fn get_media_uploads(&self) -> MediaUploadStream {
        ...
        MediaUploadStream::new(async_stream::stream! {
            ...
            debug!("Getting pending media uploads");

            match media_upload_dao.get_pending_uploads().await {
                Ok(uploads) => {
                    debug!(count = uploads.len(), "Retrieved pending media uploads");

                    for upload in uploads {
                        yield Ok(upload.request_id);
                    }
                }
                Err(err) => {
    ...
    fn remove_media_upload(&self, request_id: &str) -> MediaUploadFuture<()> {
        ...
        MediaUploadFuture::new(async move {
            ...
            debug!(request_id = %request_id, "Removing media upload");

            match media_upload_dao.remove_upload(&request_id).await {
                Ok(_) => {
                    debug!(request_id = %request_id, "Media upload removed successfully");
                    Ok(())
                },
                Err(err) => {
