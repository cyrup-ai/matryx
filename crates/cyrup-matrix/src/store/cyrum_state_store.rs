use matrix_sdk_base::store::{StateChanges, QueuedRequest};
use matrix_sdk_base::{
    ...
    store::{
        ...
        QueueWedgeError, QueuedRequestKind, SentRequestKey, StateStoreDataKey, StateStoreDataValue
    },
    ...
    ruma::{
        ...
        serde::Raw,
        MilliSecondsSinceUnixEpoch, OwnedEventId, OwnedRoomId, OwnedTransactionId, OwnedUserId,
...
pub struct StateEventPairStream(Pin<Box<dyn Stream<Item = Result<(String, RawAnySyncOrStrippedState)>> + Send>>);
...
pub struct GlobalAccountDataStream(Pin<Box<dyn Stream<Item = Result<AnyGlobalAccountDataEvent>> + Send>>);

/// A stream of room account data events
pub struct RoomAccountDataStream(Pin<Box<dyn Stream<Item = Result<AnyRoomAccountDataEvent>> + Send>>);
...
pub struct DependentQueuedRequestStream(Pin<Box<dyn Stream<Item = Result<DependentQueuedRequest>> + Send>>);
...
pub trait CyrumStateStore: Send + Sync + 'static {
