...
pub struct CyrumNotifications {
    client: Arc<MatrixClient>,
    inner: Arc<MatrixNotificationSettings>,
    runtime_handle: Handle,
}

impl CyrumNotifications {
    /// Create a new CyrumNotifications with the provided Matrix client.
    ...
    pub fn rebuild(&mut self) -> MatrixFuture<()> {
        ...
        MatrixFuture::spawn(async move {
            ...
            // The caller will need to recreate the CyrumNotifications instance
    ...
    pub fn get_room_notification_mode(&self, room_id: &RoomId) -> MatrixFuture<Option<RoomNotificationMode>> {
    ...
    pub fn set_room_notification_mode(&self, room_id: &RoomId, mode: RoomNotificationMode) -> MatrixFuture<()> {
        let room_id = room_id.to_owned();
        let inner = self.inner.clone();
        
        MatrixFuture::spawn(async move {
            inner.set_room_notification_mode(&room_id, mode).await
    ...
    pub fn reset_room_notification_mode(&self, room_id: &RoomId) -> MatrixFuture<()> {
        ...
        MatrixFuture::spawn(async move {
            inner.delete_user_defined_room_rules(&room_id).await
                .map_err(|e| crate::error::Error::Notification(NotificationError::matrix_sdk(e)))?;
            Ok(())
        })
    }
    ...
    pub fn set_room_muted(&self, room_id: &RoomId, muted: bool) -> MatrixFuture<()> {
        ...
        MatrixFuture::spawn(async move {
            if muted {
                inner.set_room_notification_mode(&room_id, RoomNotificationMode::Mute).await
                    .map_err(|e| crate::error::Error::Notification(NotificationError::matrix_sdk(e)))?;
                Ok(())
            } else {
    ...
    pub fn get_user_notification_content(&self) -> MatrixFuture<String> {
        ...
        MatrixFuture::spawn(async move {
            ...
            let raw_account_data = client.account().account_data::<PushRulesEventContent>().await
                .map_err(|e| crate::error::Error::Notification(NotificationError::matrix_sdk(e)))?;
            
            if let Some(content) = raw_account_data {
                // Serialize the content to a string
                let json = serde_json::to_string(&content)
                    .map_err(|e| crate::error::Error::Notification(NotificationError::SerializationError(e.to_string())))?;
                Ok(json)
            } else {
