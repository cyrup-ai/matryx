...
impl<T: Send + 'static> Future for DaoSession<T> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        match self.rx.poll_recv(cx) {
            ...
            Poll::Ready(Some(DaoResponse::Error(err))) => {
                Poll::Ready(Err(err))
            },
            Poll::Ready(None) => {
                Poll::Ready(Err(Error::other("Channel closed unexpectedly")))
            },
            Poll::Ready(Some(_)) => {
                Poll::Ready(Err(Error::other("Unexpected response type")))
            },
