...
impl RoomStateDao {
    ...
    /// Get a state event
    pub fn get_state_event(&self, room_id: &str, event_type: &str, state_key: &str) -> MatrixFuture<Option<Value>> {
    ...
    pub fn get_state_events(&self, room_id: &str, event_type: &str) -> MatrixFuture<Vec<(String, Value)>> {
        let dao = self.dao.clone();
        let room_id = room_id.to_string();
        let event_type = event_type.to_string();
    ...
    /// Save a state event
    pub fn save_state_event(&self, room_id: &str, event_type: &str, state_key: &str, event: Value) -> MatrixFuture<()> {
        let dao = self.dao.clone();
        let room_id = room_id.to_string();
        let event_type = event_type.to_string();
        let state_key = state_key.to_string();
    ...
    pub fn get_room_users_by_membership(&self, room_id: &str, memberships: &[String]) -> MatrixFuture<Vec<String>> {
        let dao = self.dao.clone();
        let room_id = room_id.to_string();
    ...
    pub fn get_all_room_ids(&self) -> MatrixFuture<Vec<String>> {
        ...
        MatrixFuture::spawn(async move {
            ...
            let query_result: Vec<Value> = dao.query::<Vec<Value>>(
                "SELECT DISTINCT room_id FROM room_state"
            ).await?;
    ...
    pub fn get_users_with_display_name(&self, room_id: &str, display_name: &str) -> MatrixFuture<Vec<String>> {
        let dao = self.dao.clone();
        let room_id = room_id.to_string();
