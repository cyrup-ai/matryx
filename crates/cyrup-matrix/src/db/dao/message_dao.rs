...
impl MessageDao {
    ...
    pub fn find_by_room(&self, room_id: &str, limit: usize) -> impl Future<Output = Result<MessageList>> {
    ...
    pub fn find_by_room_since(&self, room_id: &str, since: DateTime<Utc>) -> impl Future<Output = Result<MessageList>> {
        let dao = self.dao.clone();
        let room_id = room_id.to_string();
    ...
    pub fn find_by_sender(&self, sender_id: &str, limit: usize) -> impl Future<Output = Result<MessageList>> {
        let dao = self.dao.clone();
    ...
    pub fn add_reaction(&self, message_id: &str, emoji: &str, user_id: &str) -> impl Future<Output = Result<MessageItem>> {
        let dao = self.dao.clone();
        ...
        async move {
            ...
            Ok(MessageItem { message: result })
        }
    }
    
    // Remove a reaction from a message
    pub fn remove_reaction(&self, message_id: &str, emoji: &str, user_id: &str) -> impl Future<Output = Result<MessageItem>> {
        let dao = self.dao.clone();
        let message_id = message_id.to_string();
        let emoji = emoji.to_string();
        let user_id = user_id.to_string();
    ...
    pub fn search_content(&self, room_id: &str, query: &str) -> impl Future<Output = Result<MessageList>> {
        let dao = self.dao.clone();
        let room_id = room_id.to_string();
