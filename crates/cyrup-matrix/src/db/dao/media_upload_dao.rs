use crate::db::entity::MediaUpload;
...
impl MediaUploadDao {
    ...
    pub fn get_uploads(&self) -> MatrixFuture<Vec<String>> {
        ...
        MatrixFuture::spawn(async move {
            let uploads: Vec<MediaUpload> = dao.query_with_params::<Vec<MediaUpload>>(
                "SELECT * FROM media_upload WHERE status = 'started'",
                json!({})
            ).await?;
            
            let request_ids = uploads.into_iter()
                .map(|upload| upload.request_id)
                .collect();
