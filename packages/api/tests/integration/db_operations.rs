...
use chrono::Utc;
use matrix_sdk_base::ruma::events::receipt::{Receipt, ReceiptThread, ReceiptType};
...
async fn create_test_db() -> Arc<DatabaseClient> {
    let client = DatabaseClient::connect_memory()
        .await
...
fn random_room_id() -> OwnedRoomId {
    OwnedRoomId::try_from(format!("!{}:test.com", Uuid::new_v4()))
        .expect("Failed to create room ID")
}

// Helper to generate a random user ID
fn random_user_id() -> OwnedUserId {
    OwnedUserId::try_from(format!("@user_{}:test.com", Uuid::new_v4()))
        .expect("Failed to create user ID")
}

// Helper to generate a random transaction ID
fn random_transaction_id() -> OwnedTransactionId {
    OwnedTransactionId::try_from(format!("txn_{}", Uuid::new_v4()))
        .expect("Failed to create transaction ID")
}

// Helper to generate a random event ID
fn random_event_id() -> OwnedEventId {
    OwnedEventId::try_from(format!("${}:test.com", Uuid::new_v4()))
...
async fn test_account_data_dao() {
    ...
    let global_result = dao
        .get_global_account_data(global_type)
        .await
    ...
    let all_room_data = dao
        .get_all_room_account_data(&room_id)
        .await
    ...
    let all_global_data = dao
        .get_all_global_account_data()
        .await
...
async fn test_presence_dao() {
    ...
    let result = dao.get_presence_event(&user_id).await.expect("Failed to get presence");
    ...
    let multi_result = dao
        .get_presence_events(&[user_id.clone(), user_id2.clone()])
        .await
...
async fn test_receipt_dao() {
    ...
    let user_receipt = dao
        .get_user_receipt(&room_id, receipt_type, thread.clone(), &user_id)
        .await
    ...
    let event_receipts = dao
        .get_event_receipts(&room_id, receipt_type, thread.clone(), &event_id)
        .await
...
async fn test_send_queue_dao() {
    ...
    let request = QueuedRequestKind::Message { content: Box::new(content), txn_id: txn_id.clone() };
    ...
    let stored_request = dao.get_request(&room_id, &txn_id).await.expect("Failed to get request");

    assert!(stored_request.is_some(), "Expected request to be found");

    // Test getting all requests for a room
    let room_requests = dao
        .get_all_requests(&room_id)
        .await
    ...
    let removed_request = dao
        .get_request(&room_id, &txn_id)
        .await
...
async fn test_request_dependency_dao() {
    ...
    let dependents = dao
        .get_dependent_requests(&room_id, &parent_txn_id)
        .await
...
async fn test_media_upload_dao() {
    ...
    let uploads = dao.get_uploads().await.expect("Failed to get uploads");

    assert_eq!(uploads.len(), 1, "Expected exactly one upload");
    assert_eq!(uploads[0], request_id);

    // Test removing upload
    dao.remove_upload(&request_id).await.expect("Failed to remove upload");

    let remaining_uploads = dao.get_uploads().await.expect("Failed to check for remaining uploads");
...
async fn test_api_cache_dao() {
    ...
    dao.set_value(key, value).await.expect("Failed to set cache value");

    // Test getting cache value
    let stored_value = dao.get_value(key).await.expect("Failed to get cache value");

    assert_eq!(stored_value, Some(value.to_string()), "Expected to get the stored value");

    // Test removing cache value
    dao.remove_value(key).await.expect("Failed to remove cache value");

    let removed_value = dao.get_value(key).await.expect("Failed to check for removed cache value");
...
async fn test_all_daos_together() {
    ...
    room_state_dao
        .set_state_event(&room_id, "m.room.name", "", json!({"name": "Test Combined Room"}))
        .await
        .expect("Failed to set room name");

    // Test AccountDataDao
    let account_data_dao = AccountDataDao::new(client.clone());
    account_data_dao
        .set_room_account_data(&room_id, "m.fully_read", json!({"event_id": event_id.to_string()}))
        .await
        .expect("Failed to set account data");

    // Test PresenceDao
    let presence_dao = PresenceDao::new(client.clone());
    presence_dao
        .set_presence_event(&user_id, json!({"presence": "online"}))
        .await
        .expect("Failed to set presence");
    ...
    let created_at = MilliSecondsSinceUnixEpoch(Utc::now().timestamp_millis() as u64);
    send_queue_dao
        .add_request_to_queue(&room_id, txn_id.clone(), created_at, request, 1)
        .await
        .expect("Failed to add request to queue");

    // Verify all data is correctly stored
    let room_name = room_state_dao
    ...
    let account_data = account_data_dao
        .get_room_account_data(&room_id, "m.fully_read")
        .await
    ...
    let queue_request = send_queue_dao
        .get_request(&room_id, &txn_id)
        .await
        .expect("Failed to get queued request");
