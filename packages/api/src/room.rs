...
use crate::member::MatrixRoomMember;
use matrix_sdk::ruma::serde::Raw; // Add import

/// Domain-specific future for room members list operations.
pub struct RoomMembersFuture(MatrixFuture<Vec<MatrixRoomMember>>);

impl RoomMembersFuture {
    pub fn new(future: MatrixFuture<Vec<MatrixRoomMember>>) -> Self {
        Self(future)
    }
}

impl Future for RoomMembersFuture {
    type Output = MatrixResult<Vec<MatrixRoomMember>>; // Use crate's Result alias
...
impl Future for MessageFuture {
    type Output = MatrixResult<OwnedEventId>; // Use crate's Result alias
...
impl Future for RoomActionFuture {
    type Output = MatrixResult<()>; // Use crate's Result alias
...
impl Future for TimelineFuture {
    type Output = MatrixResult<Vec<AnySyncTimelineEvent>>; // Use crate's Result alias
...
impl Stream for MessageStream {
    type Item = MatrixResult<OriginalSyncRoomMessageEvent>; // Use crate's Result alias
...
impl MatrixRoom {
    /// Create a new MatrixRoom wrapping the provided Matrix Room.
    pub fn new(inner: SdkRoom) -> Self {
        Self {
            inner: Arc::new(inner),
    ...
    pub fn members(&self) -> RoomMembersFuture {
        ...
        RoomMembersFuture::new(MatrixFuture::spawn(async move {
            ...
            result.map(|members| members.into_iter().map(MatrixRoomMember::new).collect())
