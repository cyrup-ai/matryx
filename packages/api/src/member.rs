...
use matrix_sdk::ruma::{events::{StateEventType, room::message::MessageType}, MxcUri, UserId}; // Add imports
use std::sync::Arc;
...
pub struct MatrixRoomMember {
    inner: Arc<SdkRoomMember>,
}

impl MatrixRoomMember {
    /// Create a new MatrixRoomMember wrapping the provided Matrix RoomMember.
    pub fn new(inner: SdkRoomMember) -> Self {
        Self { inner: Arc::new(inner) }
    }

    /// Get the inner Matrix room member.
    pub fn inner(&self) -> &SdkRoomMember {
        &self.inner
    }
