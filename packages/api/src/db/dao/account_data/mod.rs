...
use crate::db::entity::account_data::AccountDataEntity;
...
pub struct AccountDataDao {
    dao: Dao<AccountDataEntity>,
}

impl AccountDataDao {
    const TABLE_NAME: &'static str = "account_data";

    /// Create a new AccountDataDao
    ...
    pub fn get_account_data(&self, event_type: &str) -> MatrixFuture<Option<Value>> {
        ...
        MatrixFuture::spawn(async move {
            let data: Vec<AccountDataEntity> = dao.query_with_params::<Vec<AccountDataEntity>>(
    ...
    pub fn save_account_data(
        ...
        MatrixFuture::spawn(async move {
            ...
            let updated: Vec<AccountDataEntity> =
                dao.query_with_params::<Vec<AccountDataEntity>>(query.0, query.1).await?;

            // If not updated, create new
            if updated.is_empty() {
                let account_data = AccountDataEntity {
                    id: None,
                    event_type,
                    room_id,
                    event,
    ...
    pub fn get_user_account_data_events(&self) -> MatrixFuture<Vec<Value>> {
        ...
        MatrixFuture::spawn(async move {
            let data: Vec<AccountDataEntity> = dao
                .query::<Vec<AccountDataEntity>>("SELECT * FROM account_data WHERE room_id IS NONE")
                .await?;
    ...
    pub fn get_room_account_data_events(&self, room_id: &str) -> MatrixFuture<Vec<Value>> {
        ...
        MatrixFuture::spawn(async move {
            let data: Vec<AccountDataEntity> = dao
                .query_with_params::<Vec<AccountDataEntity>>(
