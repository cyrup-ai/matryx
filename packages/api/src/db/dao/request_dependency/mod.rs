...
use crate::db::entity::request_dependency::RequestDependency;
use crate::db::generic_dao::Dao;
use crate::future::MatrixFuture;
use chrono::Utc;
use serde_json::{json, Value};

/// RequestDependency DAO
#[derive(Clone)]
...
impl RequestDependencyDao {
    const TABLE_NAME: &'static str = "request_dependency";

    /// Create a new RequestDependencyDao
    ...
    pub fn get_dependent_requests(
        &self,
        room_id: &str,
        parent_txn_id: &str,
    ) -> MatrixFuture<Vec<RequestDependency>> {
        let dao = self.dao.clone();
        let room_id = room_id.to_string();
        let parent_txn_id = parent_txn_id.to_string();
    ...
    pub fn get_all_room_dependent_requests(
        &self,
        room_id: &str,
    ) -> MatrixFuture<Vec<RequestDependency>> {
        let dao = self.dao.clone();
