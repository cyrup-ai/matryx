...
async fn test_concurrent_user_load() {
    ...
    let load_test = LoadTest::new().await
        .expect("Should be able to create load test");
    
    let report = load_test.test_concurrent_users(config.max_concurrent_users).await
    ...
    assert!(report.successful_users >= report.user_count / 2, 
            "Should handle at least half the users successfully");
...
async fn test_message_throughput_performance() {
    ...
    let report = load_test.test_message_throughput(message_count).await
    ...
    assert!(success_rate > 80.0, "Should send >80% of messages successfully, got {:.1}%", success_rate);
...
async fn test_sync_endpoint_performance() {
    ...
    assert!(report.average_response_time.as_secs() < 5, 
            "Sync should be reasonably fast, got {:.2}s", report.average_response_time.as_secs_f64());
