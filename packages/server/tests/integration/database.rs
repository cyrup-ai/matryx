...
impl DatabaseTestHarness {
    ...
    pub async fn test_livequery_notifications(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        match notification_result {
            Ok(Some(notification)) => {
                ...
                assert!(notification.to_string().contains("CREATE"));
                Ok(())
            },
    ...
    pub async fn test_matrix_entity_coverage(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let tables_to_test = vec![
            "users", "rooms", "room_state_events", "room_timeline_events",
            "devices", "access_tokens", "room_memberships", "user_filters",
            "media_files", "push_rules", "account_data", "presence_events"
        ];
        ...
        for table in tables_to_test {
            ...
            assert!(result.is_empty() || !result.is_empty(), "Table {} should be accessible", table);
        }
        
        Ok(())
    }
    
    pub async fn test_authentication_context_preservation(&self) -> Result<(), Box<dyn std::error::Error>> {
...
mod tests {
    ...
    async fn test_database_harness_initialization() {
        ...
        let result: Vec<Value> = harness.db
