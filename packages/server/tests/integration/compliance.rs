        ...
        match output {
            Ok(output) => {
                ...
                Ok(SyTestResults {
                    ...
                    skipped: 0,
                    failures,
                    status: if failed == 0 { "PASSED".to_string() } else { "FAILED".to_string() },
                })
            },
            Err(e) => {
                Ok(SyTestResults {
...
impl EndpointComplianceTest {
    ...
    pub async fn setup_authenticated_user(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        let (_, access_token) = create_test_user(&self.server, "compliance_test_user", "test_password").await?;
        self.access_token = Some(access_token);
        Ok(())
    }
    
    pub async fn test_all_endpoints(&mut self) -> Result<ComplianceReport, Box<dyn std::error::Error>> {
    ...
    async fn test_foundation_api(&self) -> Result<ComplianceReport, Box<dyn std::error::Error>> {
        ...
        let response = self.server.test_endpoint("GET", "/_matrix/client/v3/capabilities", None).await;
        ...
        if let Some(token) = &self.access_token {
            ...
            let response = self.server.test_authenticated_endpoint("POST", "/_matrix/client/v3/user/@compliance_test_user:test.localhost/filter", token, Some(filter_body)).await;
            report.add_test("user_filter", response.status_code() == 200);
            
            // Test GET /_matrix/client/v3/sync
            let response = self.server.test_authenticated_endpoint("GET", "/_matrix/client/v3/sync", token, None).await;
    ...
    async fn test_rooms_users_api(&self) -> Result<ComplianceReport, Box<dyn std::error::Error>> {
        ...
        if let Some(token) = &self.access_token {
            ...
            let response = self.server.test_authenticated_endpoint("POST", "/_matrix/client/v3/createRoom", token, Some(room_body)).await;
            report.add_test("create_room", response.status_code() == 200);
            
            // Test GET /_matrix/client/v3/joined_rooms
            let response = self.server.test_authenticated_endpoint("GET", "/_matrix/client/v3/joined_rooms", token, None).await;
            report.add_test("joined_rooms", response.status_code() == 200);
            
            // Test GET /_matrix/client/v3/publicRooms
            let response = self.server.test_authenticated_endpoint("GET", "/_matrix/client/v3/publicRooms", token, None).await;
    ...
    async fn test_messaging_api(&self) -> Result<ComplianceReport, Box<dyn std::error::Error>> {
        ...
        if let Some(token) = &self.access_token {
            ...
            let response = self.server.test_authenticated_endpoint("PUT", &path, token, Some(message_body)).await;
    ...
    async fn test_security_api(&self) -> Result<ComplianceReport, Box<dyn std::error::Error>> {
        ...
        if let Some(token) = &self.access_token {
            ...
            let response = self.server.test_authenticated_endpoint("GET", "/_matrix/client/v3/devices", token, None).await;
            report.add_test("devices", response.status_code() == 200);
            
            // Test GET /_matrix/client/v3/account/whoami
            let response = self.server.test_authenticated_endpoint("GET", "/_matrix/client/v3/account/whoami", token, None).await;
...
mod tests {
    ...
    async fn test_endpoint_compliance_foundation() {
        ...
        println!("Foundation API compliance: {:.1}% ({}/{})", report.pass_rate(), report.passed, report.total);
    }
    ...
    async fn test_full_endpoint_compliance() {
        ...
        println!("Overall API compliance: {:.1}% ({}/{})", report.pass_rate(), report.passed, report.total);
        ...
        assert!(report.pass_rate() > 50.0, "Compliance rate should be > 50%, got {:.1}%", report.pass_rate());
    }
    ...
    async fn test_sytest_runner() {
        ...
        println!("SyTest results: {} (passed: {}, failed: {}, skipped: {})", 
        ...
        assert!(results.status == "SyTest not available - skipped" || 
