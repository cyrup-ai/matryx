...
pub struct FederationTest {
    server1: MatrixTestServer,
    server2: MatrixTestServer,
}
...
impl FederationTest {
    ...
    pub async fn test_cross_server_messaging(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let response = self.server1.test_authenticated_endpoint("POST", &path, &user1_token, Some(invite_body)).await;
        ...
        assert!(response.status_code() == 200 || response.status_code() == 403, 
                "Invite should succeed or fail gracefully, got {}", response.status_code());
    ...
    pub async fn test_federation_authentication(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let response = self.server1.test_endpoint("GET", "/_matrix/federation/v1/version", None).await;
    ...
    pub async fn test_federation_event_handling(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for endpoint in federation_endpoints {
            ...
            assert!(response.status_code() != 404, 
                    "Federation endpoint {} should exist", endpoint);
        }
