...
async fn test_matrix_compliance_suite() {
    ...
    assert!(report.pass_rate() > 60.0, 
            "Compliance rate should be > 60%, got {:.1}%", report.pass_rate());
...
async fn test_sytest_compliance() {
    ...
    let test_server = integration::MatrixTestServer::new().await;
    let sytest_runner = SyTestRunner::new(&test_server.base_url)
        .expect("Should be able to create SyTest runner");
