...
#[tokio::test]
async fn test_capabilities_endpoint_compliance() {
    ...
    assert!(
        caps["m.set_displayname"]["enabled"]
            .as_bool()
            .expect("Missing displayname capability")
    );
    assert!(
        caps["m.set_avatar_url"]["enabled"]
            .as_bool()
            .expect("Missing avatar capability")
    );
    assert!(
    ...
    assert!(
        caps["org.matrix.lazy_loading"]["enabled"]
            .as_bool()
            .expect("Missing lazy loading capability")
    );
    ...
    assert!(
        caps["org.matrix.threading"]["enabled"]
            .as_bool()
            .expect("Missing threading capability")
    );
}
...
async fn test_filter_crud_operations() {
    ...
    assert!(filter.room.as_ref().expect("Room filter should exist").timeline.is_some());
    assert!(
        filter
            .room
            .as_ref()
            .expect("Room filter should exist")
            .timeline
            .as_ref()
            .expect("Timeline filter should exist")
            .lazy_load_members
    );
    ...
    let deserialized: MatrixFilter =
        serde_json::from_str(&serialized).expect("Filter should deserialize");
    ...
    assert_eq!(
        deserialized.room.as_ref().expect("Room filter should exist").rooms,
        filter.room.as_ref().expect("Room filter should exist").rooms
    );
    ...
    let retrieved_filter = filter_repo
        .get_by_id(&filter_id)
        .await
        .expect("Filter retrieval should succeed")
    ...
    // Cleanup
    test_db.cleanup().await.expect("Test cleanup should succeed");
}
...
async fn test_sync_with_filter() {
    ...
    let filter_json = serde_json::to_string(&filter).expect("Filter should serialize to JSON");

    // Verify filter JSON structure
    let parsed_filter: Value =
        serde_json::from_str(&filter_json).expect("Filter JSON should be valid");
...
#[tokio::test]
async fn test_lazy_loading_members() {
...
async fn test_event_filtering() {
    ...
    assert!(
        timeline
            ...
            .not_senders
            .as_ref()
            .expect("Not senders should exist")
            .contains(&"@spam:example.com".to_string())
    );
    assert_eq!(timeline.contains_url, Some(true));

    // Verify event fields filtering
    let event_fields = event_filter.event_fields.as_ref().expect("Event fields should exist");
...
async fn test_room_filtering() {
    ...
    let room_filter = inclusion_filter.room.as_ref().expect("Room filter should exist");
    assert!(
        room_filter
            .rooms
            .as_ref()
            .expect("Rooms should exist")
            .contains(&"!important:example.com".to_string())
    );
    ...
    let exclusion_room_filter = exclusion_filter.room.as_ref().expect("Room filter should exist");
    assert!(
        exclusion_room_filter
            .not_rooms
            .as_ref()
            .expect("Not rooms should exist")
            .contains(&"!spam:example.com".to_string())
    );
...
#[tokio::test]
async fn test_presence_and_account_data_filtering() {
    ...
    let presence_filter = comprehensive_filter
        .presence
        .as_ref()
        .expect("Presence filter should exist");
    ...
    let account_data_filter = comprehensive_filter
        .account_data
        .as_ref()
        .expect("Account data filter should exist");
