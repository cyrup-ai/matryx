...
impl LoadTest {
    pub async fn new() -> Result<Self, Box<dyn std::error::Error + Send>> {
        ...
        Ok(Self { server, metrics })
    }

    pub async fn test_concurrent_users(
        &self,
        user_count: u32,
    ...
    pub async fn test_message_throughput(
        &self,
        message_count: u32,
        ...
        for i in 0..message_count {
            ...
            let response = self
                .server
                .test_authenticated_endpoint("PUT", &path, &access_token, Some(message_body))
                .await;
    ...
    pub async fn test_sync_performance(
        &self,
    ) -> Result<SyncPerformanceReport, Box<dyn std::error::Error>> {
        // Create test user
        let (user_id, access_token) =
...
async fn simulate_user_session(
    ...
    metrics
        ...
        .fetch_add(session_duration.as_millis() as u64, Ordering::Relaxed);
