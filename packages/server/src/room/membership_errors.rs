...
impl fmt::Display for MembershipError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            MembershipError::InsufficientPermissions { action, required_level, user_level, .. } => {
                write!(f, "Insufficient permissions for {}: required level {}, user has {}", 
            ...
            MembershipError::MembershipAlreadyExists { user_id, current_membership, requested_membership, .. } => {
                write!(f, "User {} already has membership {} (requested {})", 
            ...
            MembershipError::FederationTimeout { server_name, timeout_ms, operation } => {
                write!(f, "Federation timeout for {} on {}: {}ms", operation, server_name, timeout_ms)
            }
            ...
            MembershipError::ConflictingMembershipChange { user_id, conflicting_event_id, .. } => {
                write!(f, "Conflicting membership change for {} (conflict: {})", user_id, conflicting_event_id)
            }
            MembershipError::InconsistentRoomState { room_id, details } => {
...
mod tests {
    ...
    // These tests would cover all error types, status codes, and Matrix compliance
}