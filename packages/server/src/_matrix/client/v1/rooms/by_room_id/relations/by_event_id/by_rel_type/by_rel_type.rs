...
pub async fn get(
    ...
    // Get related events with specific relation type
    let relations = get_event_relations_by_type(
        &state,
        &room_id,
        &event_id,
        &rel_type,
    ...
    // Get reaction aggregations if this is an annotation relation
    let aggregations = if rel_type == "m.annotation" {
        get_reaction_aggregations(&state, &event_id).await?
    } else {
        HashMap::new()
    };
...
async fn get_event_relations_by_type(
    state: &AppState,
    room_id: &str,
    event_id: &str,
    rel_type: &str,
    ...
    let mut query = r#"
        ...
        AND (e.content.m.relates_to.rel_type = $rel_type OR e.content['m.relates_to'].rel_type = $rel_type)
    "#.to_string();
