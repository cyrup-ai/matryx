...
use matryx_surrealdb::repository::{UserRepository, SessionRepository};
    ...
    if let Some(uia_auth) = request.auth {
        ...
        match uia_service.process_auth(&session_id, uia_auth).await {
            Ok(_) => {
                ...
                let mut user = user_repo.get_by_id(&authenticated_user.user_id).await
                    .map_err(|_| MatrixError::Unknown.into_response())?
                ...
                if request.logout_devices.unwrap_or(false) {
                    ...
                    session_repo.deactivate_all_user_sessions(&authenticated_user.user_id).await
                        .map_err(|_| MatrixError::Unknown.into_response())?;
                }
