...
pub async fn post(
    ...
    info!("Password reset email token request for email: {} (attempt: {})", 
    ...
    if third_party_repo.find_user_by_third_party("email", &request.email).await
        .map_err(|_| MatrixError::Unknown.into_response())?.is_none() {
    ...
    // TODO: Send actual password reset email with validation token
    
    let response = PasswordEmailRequestTokenResponse {
        sid: session.session_id,
        submit_url: Some(format!("{}/_matrix/client/v3/account/password/email/submit_token", 
