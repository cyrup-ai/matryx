...
pub async fn get(State(state): State<AppState>) -> Result<Json<Value>, StatusCode> {
    ...
    let overall_status = if app_health.database_connected {
        if app_health.lazy_loading.as_ref().is_none_or(|h| h.is_healthy())
            && app_health.memory.as_ref().is_none_or(|h| h.is_healthy())
        {
            "healthy"
        } else {
            "degraded"
...
pub async fn post(
    ...
    let record_to_monitoring = payload.get("record").and_then(|v| v.as_bool()).unwrap_or(false);

    // Perform health check
    let database_health = state
