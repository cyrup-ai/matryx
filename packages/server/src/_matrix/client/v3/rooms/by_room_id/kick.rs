...
pub async fn post(
    ...
    // Check kicker's power level and permission to kick
    if !can_user_kick(&state, &room, &kicker_id, &request.user_id).await? {
        warn!(
    ...
    // Get event context for the kick event (which is actually a leave event)
    ...
    let auth_events = get_auth_events_for_kick(&state, &room_id, &kicker_id)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;

    // Create leave membership event for the kicked user
    ...
    .map_err(|e| {
        error!(
            "Failed to create kick event from {} to kick {} in room {}: {}",
            kicker_id, request.user_id, room_id, e
        );
        StatusCode::INTERNAL_SERVER_ERROR
    })?;

    info!(
        "Successfully created kick event {} from {} to kick {} in room {}",
        kick_event_id, kicker_id, request.user_id, room_id
    );
...
async fn can_user_kick(
    ...
    // Get the kicker's power level
    let kicker_power_level =
        if let Some(users) = power_levels.get("users").and_then(|u| u.as_object()) {
            users.get(kicker_id).and_then(|p| p.as_i64()).unwrap_or(0)
        } else {
            0
        };
    ...
    // Get the required power level to kick (default 50 per Matrix spec)
    let required_kick_level = power_levels.get("kick").and_then(|k| k.as_i64()).unwrap_or(50);

    // Kicker must have sufficient power level to kick AND must have higher power level than target
    Ok(kicker_power_level >= required_kick_level && kicker_power_level > target_power_level)
}
...
async fn get_auth_events_for_kick(
    state: &AppState,
    room_id: &str,
    kicker_id: &str,
) -> Result<Vec<String>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    let query = r#"
        ...
        AND ((event_type = 'm.room.create' AND state_key = '')
             ...
             OR (event_type = 'm.room.member' AND state_key = $kicker_id))
    ...
    let mut result = state
        ...
        .bind(("kicker_id", kicker_id.to_string()))
        .await?;
...
async fn create_membership_event(
    ...
    let membership_record = Membership {
        ...
        invited_by: None, // Not applicable for kick events
