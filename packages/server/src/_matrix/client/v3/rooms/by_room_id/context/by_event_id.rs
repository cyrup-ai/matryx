...
pub async fn get(
    ...
    let matrix_filter = if let Some(filter_id) = params.filter.as_ref() {
        ...
        let filter_repo = matryx_surrealdb::repository::filter::FilterRepository::new(state.db.clone());
        match filter_repo.get_by_id(filter_id).await {
            Ok(Some(filter)) => {
                info!("Applied filter {} to context request for event {} in room {}", filter_id, event_id, room_id);
                Some(filter)
            },
            Ok(None) => {
            ...
            if let Some(ref filter) = matrix_filter {
                ...
                if let Some(ref room_filter) = filter.room {
                    if let Some(ref timeline_filter) = room_filter.timeline {
                        ...
                        let events_before_json: Vec<serde_json::Value> = context_response.events_before
                            .into_iter()
                            .map(|e| serde_json::to_value(e).unwrap_or_default())
                            .collect();
                        let filtered_events_before = apply_context_event_filter(events_before_json, &timeline_filter.base);
                        context_response.events_before = filtered_events_before
                            .into_iter()
                            .map(|v| serde_json::from_value(v).unwrap_or_default())
                            .collect();

                        let events_after_json: Vec<serde_json::Value> = context_response.events_after
                            .into_iter()
                            .map(|e| serde_json::to_value(e).unwrap_or_default())
                            .collect();
                        let filtered_events_after = apply_context_event_filter(events_after_json, &timeline_filter.base);
                    ...
                    if let Some(ref state_filter) = room_filter.state {
                        ...
                        let state_json: Vec<serde_json::Value> = context_response.state
                            .into_iter()
                            .map(|e| serde_json::to_value(e).unwrap_or_default())
                            .collect();
                        let filtered_state = apply_context_event_filter(state_json, &state_filter.base);
                ...
                if let Some(ref event_fields) = filter.event_fields {
                    let events_before_json: Vec<serde_json::Value> = context_response.events_before
                        ...
                        .map(|e| serde_json::to_value(e).unwrap_or_default())
                        .collect();
                    let filtered_events_before = self::apply_event_fields_filter(events_before_json, event_fields);
                    context_response.events_before = filtered_events_before
                        .into_iter()
                        .map(|v| serde_json::from_value(v).unwrap_or_default())
                        .collect();
                    ...
                    let events_after_json: Vec<serde_json::Value> = context_response.events_after
                        ...
                        .map(|e| serde_json::to_value(e).unwrap_or_default())
                        .collect();
                    let filtered_events_after = self::apply_event_fields_filter(events_after_json, event_fields);
                    context_response.events_after = filtered_events_after
                        .into_iter()
                        .map(|v| serde_json::from_value(v).unwrap_or_default())
                        .collect();
                    ...
                    if let Some(ref mut event) = context_response.event {
                        ...
                        let filtered_event_json = self::apply_event_fields_filter_single(event_json, event_fields);
...
fn apply_event_fields_filter(
    ...
    events.into_iter().map(|event| {
        apply_event_fields_filter_single(event, event_fields)
    }).collect()
}
...
fn apply_event_fields_filter_single(
    ...
    if let serde_json::Value::Object(ref mut event_obj) = event {
        ...
        for field_path in event_fields {
            ...
            if let Some(value) = get_nested_value(&serde_json::Value::Object(original_event.clone()), &path_parts) {
...
fn set_nested_value(
    ...
    if remaining_path.is_empty() {
        ...
        let nested_obj = obj.entry(key.to_string())
