    ...
    if let Some(room_events_criteria) = request.search_categories.room_events {
        ...
        let search_criteria = SearchCriteria {
            ...
            room_events: Some(matryx_surrealdb::repository::search::RoomEventsCriteria {
                ...
                filter: room_events_criteria.filter.as_ref().map(|f| {
                    RoomEventFilter {
                        limit: f.limit,
                ...
                event_context: room_events_criteria.event_context.as_ref().map(|ec| {
                    EventContext {
                ...
                groupings: room_events_criteria.groupings.as_ref().map(|g| {
                    SearchGroupings {
