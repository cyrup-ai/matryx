...
async fn fetch_url_preview(
    ...
    if let Ok(title_regex) = regex::Regex::new(r#"<meta[^>]+property="og:title"[^>]+content="([^"]*)"[^>]*>"#)
        && let Some(cap) = title_regex.captures(&html) {
            title = Some(cap[1].to_string());
        }
    
    if let Ok(desc_regex) = regex::Regex::new(r#"<meta[^>]+property="og:description"[^>]+content="([^"]*)"[^>]*>"#)
        && let Some(cap) = desc_regex.captures(&html) {
            description = Some(cap[1].to_string());
        }
    
    if let Ok(img_regex) = regex::Regex::new(r#"<meta[^>]+property="og:image"[^>]+content="([^"]*)"[^>]*>"#)
    ...
    if description.is_none()
        && let Ok(meta_desc_regex) = regex::Regex::new(r#"<meta[^>]+name="description"[^>]+content="([^"]*)"[^>]*>"#)
    ...
    if let Some(img_url) = image_url {
        ...
        let absolute_img_url = if img_url.starts_with("http://") || img_url.starts_with("https://") {
            ...
            if let Ok(parsed_url) = url::Url::parse(url) {
                format!("{}://{}{}", parsed_url.scheme(), parsed_url.host_str().unwrap_or(""), img_url)
            } else {
                img_url // Fallback
            }
        } else {
