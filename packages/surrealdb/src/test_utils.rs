...
pub fn create_test_db() -> Result<Surreal<Any>, TestUtilsError> {
    let rt = tokio::runtime::Runtime::new()
        .map_err(TestUtilsError::RuntimeCreation)?;
    
    rt.block_on(async {
        create_test_db_async().await
    })
}
...
impl Default for TestDatabaseConfig {
    fn default() -> Self {
        Self {
            backend_url: "mem://".to_string(),  // In-memory for speed
...
impl TestDatabaseConfig {
    ...
    pub fn performance() -> Self {
        Self {
            backend_url: "surrealkv://test_performance.db".to_string(),
            enable_logging: true,
...
impl TestDatabase {
    ...
    pub async fn with_config(config: TestDatabaseConfig) -> Result<Self, TestUtilsError> {
        ...
        let db = surrealdb::engine::any::connect(&config.backend_url).await
        ...
        db.query(migration_sql).await
            .map_err(|e| TestUtilsError::SchemaInitialization { 
                message: e.to_string() 
            })?;
    ...
    pub async fn seed_test_rooms(&self, count: usize, creator_user_id: &str) -> Result<Vec<String>, TestUtilsError> {
    ...
    pub async fn seed_test_devices(&self, user_id: &str, count: usize) -> Result<Vec<String>, TestUtilsError> {
    ...
    pub async fn cleanup(&self) -> Result<(), TestUtilsError> {
        self.db.query("REMOVE DATABASE $db")
            .bind(("db", self.database_name.clone()))
            .await
            .map_err(|e| TestUtilsError::CleanupFailed { 
                message: e.to_string() 
            })?;
        Ok(())
    }
    ...
    fn drop(&mut self) {
        ...
        if let Some(rt) = &self.runtime {
            ...
            let _ = rt.spawn(async move {
                let _ = db.query("REMOVE DATABASE $db")
                    .bind(("db", db_name))
                    .await;
            });
        }
...
/// Create test database with custom configuration
pub async fn create_test_database_with_config(config: TestDatabaseConfig) -> Result<TestDatabase, TestUtilsError> {
