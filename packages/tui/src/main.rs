//! # maxtryx
//!
//! The maxtryx client loops over user input and commands, and turns them into actions, [some of
//! which][maxtryxAction] are specific to maxtryx, and [some of which][Action] come from [modalkit]. When
//! adding new functionality, you will usually want to extend [maxtryxAction] or one of its variants
...
use crate::windows::MatrixWindow;
...
use crate::{
    ...
    config::{ApplicationSettings, maxtryx},
    windows::maxtryxWindow,
...
) -> maxtryxResult<WindowLayoutDescription<maxtryxInfo>> {
    let desc = match layout {
        config::WindowLayout::Window { window } => {
            ...
            let window = match window {
                ...
                config::WindowPath::RoomId(room_id) => maxtryxId::Room(room_id, None),
...
fn restore_layout(
    ...
    store: &mut ProgramStore,
) -> maxtryxResult<FocusList<WindowLayoutState<maxtryxWindow, maxtryxInfo>>> {
    let layout = std::fs::read(&settings.layout_json)?;
    let tabs: TabbedLayoutDescription<maxtryxInfo> =
        serde_json::from_slice(&layout).map_err(maxtryxError::from)?;
...
fn setup_screen(
    settings: ApplicationSettings,
    store: &mut ProgramStore,
) -> maxtryxResult<ScreenState<maxtryxWindow, maxtryxInfo>> {
    ...
    match settings.layout {
        ...
        config::Layout::Config { tabs } => {
            ...
            for tab in tabs.into_iter() {
                ...
                let tab = tab.to_layout(area, store)?;
                list.push(tab);
            }
    ...
    let win = settings
        .tunables
        .default_room
        .and_then(|room| maxtryxWindow::find(room, store).ok())
        .or_else(|| maxtryxWindow::open(maxtryxId::Welcome, store).ok())
        .unwrap();
...
struct Application {
    ...
    screen: ScreenState<maxtryxWindow, maxtryxInfo>,
    ...
    last_layout: Option<TabbedLayoutDescription<maxtryxInfo>>,
...
impl Application {
    ...
    fn redraw(&mut self, full: bool, store: &mut ProgramStore) -> Result<(), std::io::Error> {
        ...
        term.draw(|f| {
            ...
            let screen = Screen::new(store)
                ...
                .borders(true)
                .focus(focused);
            // Use render_widget instead of render_stateful_widget due to version incompatibilities
            screen.render(f, area, sstate);
    ...
    async fn action_run(
        ...
        let info = match action {
            ...
            // Simple delegations.
            Action::Application(act) => self.maxtryx_run(act, ctx, store).await?,
            ...
            Action::Suspend => {
                // Disable raw mode and exit alternate screen
                ...
                // Re-enable raw mode and return to alternate screen
    ...
    async fn maxtryx_run(
        &mut self,
        action: maxtryxAction,
        ctx: ProgramContext,
        store: &mut ProgramStore,
    ) -> maxtryxResult<EditInfo> {
        if action.scribbles() {
        ...
        let info = match action {
            ...
            maxtryxAction::ToggleScrollbackFocus => {
            ...
            maxtryxAction::Keys(act) => self.keys_command(act, ctx, store).await?,
    ...
    async fn homeserver_command(
        ...
        match action {
            ...
            HomeserverAction::Logout(user, false) => {
                ...
                let act = maxtryxAction::from(HomeserverAction::Logout(user, true));
...
async fn login(worker: &Requester, settings: &ApplicationSettings) -> maxtryxResult<()> {
...
async fn check_import_keys(
    settings: &ApplicationSettings,
) -> maxtryxResult<Option<(temp_dir::TempDir, String)>> {
...
fn setup_tty(title: &str, enable_enhanced_keys: bool) -> std::io::Result<()> {
    let title = format!("maxtryx ({})", title);
...
async fn run(settings: ApplicationSettings) -> maxtryxResult<()> {
    ...
    match res {
        Err(UIError::Application(maxtryxError::Matrix(e))) => {
...
fn main() -> maxtryxResult<()> {
    // Parse command-line flags.
    let maxtryx = maxtryx::parse();

    // Load configuration and set up the Matrix SDK.
    let settings = ApplicationSettings::load(maxtryx).unwrap_or_else(print_exit);
    ...
    let log_prefix = format!("maxtryx-log-{}", settings.profile_name);
