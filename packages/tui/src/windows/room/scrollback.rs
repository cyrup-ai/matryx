...
fn no_msgs() -> EditError<maxtryxInfo> {
...
impl ScrollbackState {
    pub fn new(room_id: OwnedRoomId, thread: Option<OwnedEventId>) -> ScrollbackState {
        let id = maxtryxBufferId::Room(room_id.to_owned(), thread.clone(), RoomFocus::Scrollback);
...
impl WindowOps<maxtryxInfo> for ScrollbackState {
...
impl EditorActions<ProgramContext, ProgramStore, maxtryxInfo> for ScrollbackState {
    fn edit(
        &mut self,
...
impl Editable<ProgramContext, ProgramStore, maxtryxInfo> for ScrollbackState {
    fn editor_command(
        &mut self,
        act: &EditorAction,
        ctx: &ProgramContext,
...
impl Jumpable<ProgramContext, maxtryxInfo> for ScrollbackState {
    fn jump(
        &mut self,
        list: PositionList,
        dir: MoveDir1D,
...
impl Promptable<ProgramContext, ProgramStore, maxtryxInfo> for ScrollbackState {
    fn prompt(
        &mut self,
        act: &PromptAction,
        ctx: &ProgramContext,
        store: &mut ProgramStore,
    ) -> EditResult<Vec<(Action<maxtryxInfo>, ProgramContext)>, maxtryxInfo> {
        let info = store.application.get_room_info(self.room_id.clone());
        let thread = self.get_thread(info).ok_or_else(no_msgs)?;
        ...
        match act {
            PromptAction::Submit => {
                if self.thread.is_some() {
                    ...
                    let id = maxtryxId::Room(room_id, Some(root));
...
impl ScrollActions<ProgramContext, ProgramStore, maxtryxInfo> for ScrollbackState {
    fn dirscroll(
        &mut self,
        dir: MoveDir2D,
        size: ScrollSize,
...
impl Scrollable<ProgramContext, ProgramStore, maxtryxInfo> for ScrollbackState {
    fn scroll(
        &mut self,
        style: &ScrollStyle,
        ctx: &ProgramContext,
...
impl Searchable<ProgramContext, ProgramStore, maxtryxInfo> for ScrollbackState {
    fn search(
        &mut self,
        dir: MoveDirMod,
        count: Count,
