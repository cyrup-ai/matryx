...
use maxtryx_matrix::client::MatrixClient;
use maxtryx_matrix::room::MatrixRoom;
use maxtryx_matrix::sync::MatrixSync;
use maxtryx_matrix::encryption::MatrixEncryption;
use maxtryx_matrix::error::Result as MatrixResult;
use maxtryx_matrix::future::{MatrixFuture, MatrixStream};
...
const maxtryx_DEVICE_NAME: &str = "maxtryx";
const maxtryx_USER_AGENT: &str = "maxtryx";
const MIN_MSG_LOAD: u32 = 50;

type MessageFetchResult =
    maxtryxResult<(Option<String>, Vec<(AnyMessageLikeEvent, Vec<OwnedUserId>)>)>;

fn initial_devname() -> String {
    format!("{} on {}", maxtryx_DEVICE_NAME, gethostname().to_string_lossy())
}
...
pub async fn create_room(
    ...
    match rt {
        ...
        CreateRoomType::Space => {
            ...
            let raw_cc = Raw::new(&cc).map_err(maxtryxError::from)?;
    ...
    if flags.contains(CreateRoomFlags::ENCRYPTED) {
        ...
        let encr_raw = Raw::new(&encr).map_err(maxtryxError::from)?;
    ...
    let resp = client.create_room(request).await.map_err(maxtryxError::from)?;

    if is_direct {
        if let Some(room) = client.get_room(resp.room_id()) {
            room.set_is_direct(true).await.map_err(maxtryxError::from)?;
        } else {
            error!(
...
async fn run_plan(client: &MatrixClient, store: &AsyncProgramStore, plan: Plan, permits: &Semaphore) {
...
async fn load_older_one(
    ...
    // directly as MatrixRoom doesn't yet have full support for all the options we need
    if let Some(maxtryx_room) = client.get_room(room_id) {
        let matrix_room = maxtryx_room.inner();
        ...
        let Messages { end, chunk, .. } = matrix_room.messages(opts).await.map_err(maxtryxError::from)?;

        let mut msgs = vec![];

        for ev in chunk.into_iter() {
            let deserialized = ev.into_raw().deserialize().map_err(maxtryxError::Serde)?;
        ...
        Err(maxtryxError::UnknownRoom(room_id.to_owned()).into())
    }
}
...
async fn load_older(client: &MatrixClient, store: &AsyncProgramStore) -> usize {
...
async fn members_load(client: &MatrixClient, room_id: &RoomId) -> maxtryxResult<Vec<RoomMember>> {
    if let Some(room) = client.get_room(room_id) {
        // Get members using MatrixRoom's members method
        match room.members().await {
            Ok(maxtryx_members) => {
                // Convert MatrixRoomMember to RoomMember for compatibility
                let matrix_members: Vec<RoomMember> = maxtryx_members.into_iter()
                    .map(|m| m.inner().clone())
                    .collect();
            ...
            Err(e) => {
                Err(maxtryxError::MatrixSdk(e.to_string()).into())
            }
        }
    } else {
        Err(maxtryxError::UnknownRoom(room_id.to_owned()).into())
...
fn members_insert(
    room_id: OwnedRoomId,
    res: maxtryxResult<Vec<RoomMember>>,
...
async fn load_older_forever(client: &MatrixClient, store: &AsyncProgramStore) {
...
async fn refresh_rooms(client: &MatrixClient, store: &AsyncProgramStore) {
    let mut names = vec![];
    ...
    // Process joined rooms using MatrixClient
    for maxtryx_room in client.joined_rooms() {
        let room_id = maxtryx_room.room_id().to_owned();
        let name = maxtryx_room.name().unwrap_or_else(|| room_id.to_string());
        
        // Get tags - for now, we have to use the inner matrix room for compatibility
        let tags = maxtryx_room.inner().tags().await.unwrap_or_default();
        
        // Note the room name
        names.push((room_id.clone(), name));
        
        // Add to the appropriate list
        let matrix_room = maxtryx_room.inner().clone();
        
        if maxtryx_room.is_direct() {
            dms.push(Arc::new((matrix_room, tags)));
        } else if maxtryx_room.inner().is_space() {
    ...
    // as MatrixClient doesn't have invited_rooms method yet
...
async fn refresh_rooms_forever(client: &MatrixClient, store: &AsyncProgramStore) {
...
async fn send_receipts_forever(client: &MatrixClient, store: &AsyncProgramStore) {
    let mut interval = tokio::time::interval(Duration::from_secs(2));
    ...
    loop {
        ...
        for (room_id, new_receipt) in updates {
            if let Some(room) = client.get_room(room_id.as_ref()) {
                // MatrixRoom doesn't have a direct send_single_receipt method,
...
pub async fn do_first_sync(client: &MatrixClient, store: &AsyncProgramStore) -> Result<(), MatrixError> {
    // Get MatrixSync from the client
    let sync = client.sync();
    
    // Perform an initial sync - MatrixSync handles the details internally
...
pub enum WorkerTask {
    ...
    Login(LoginStyle, ClientReply<maxtryxResult<EditInfo>>),
    Logout(String, ClientReply<maxtryxResult<EditInfo>>),
    GetInviter(MatrixRoom, ClientReply<maxtryxResult<Option<RoomMember>>>),
    GetRoom(OwnedRoomId, ClientReply<maxtryxResult<FetchedRoom>>),
    JoinRoom(String, ClientReply<maxtryxResult<OwnedRoomId>>),
    Members(OwnedRoomId, ClientReply<maxtryxResult<Vec<RoomMember>>>),
    SpaceMembers(OwnedRoomId, ClientReply<maxtryxResult<Vec<OwnedRoomId>>>),
    TypingNotice(OwnedRoomId),
    Verify(VerifyAction, SasVerification, ClientReply<maxtryxResult<EditInfo>>),
    VerifyRequest(OwnedUserId, ClientReply<maxtryxResult<EditInfo>>),
}
...
/// Create a new MatrixClient wrapping a matrix-sdk Client
pub fn create_maxtryx_client(client: Client) -> MatrixClient {
    MatrixClient::from_client(client)
}
...
impl Requester {
    ...
    pub fn login(&self, style: LoginStyle) -> maxtryxResult<EditInfo> {
        let (reply, response) = oneshot();
    ...
    pub fn logout(&self, user_id: String) -> maxtryxResult<EditInfo> {
        let (reply, response) = oneshot();
    ...
    pub fn get_inviter(&self, invite: MatrixRoom) -> maxtryxResult<Option<RoomMember>> {
        let (reply, response) = oneshot();
    ...
    pub fn get_room(&self, room_id: OwnedRoomId) -> maxtryxResult<FetchedRoom> {
        let (reply, response) = oneshot();
    ...
    pub fn join_room(&self, name: String) -> maxtryxResult<OwnedRoomId> {
        let (reply, response) = oneshot();
    ...
    pub fn members(&self, room_id: OwnedRoomId) -> maxtryxResult<Vec<RoomMember>> {
        let (reply, response) = oneshot();
    ...
    pub fn space_members(&self, space: OwnedRoomId) -> maxtryxResult<Vec<OwnedRoomId>> {
        let (reply, response) = oneshot();
    ...
    pub fn verify(&self, act: VerifyAction, sas: SasVerification) -> maxtryxResult<EditInfo> {
        let (reply, response) = oneshot();
    ...
    pub fn verify_request(&self, user_id: OwnedUserId) -> maxtryxResult<EditInfo> {
        let (reply, response) = oneshot();
...
impl ClientWorker {
    pub async fn spawn(client: Client, settings: ApplicationSettings) -> Requester {
        ...
        // Create MatrixClient and related components
        let maxtryx_client = create_maxtryx_client(client.clone());
        let maxtryx_encryption = maxtryx_client.encryption();
        let maxtryx_sync = maxtryx_client.sync();

        let mut worker = ClientWorker {
            initialized: false,
            settings,
            client: maxtryx_client.clone(),
            encryption: maxtryx_encryption,
            sync: maxtryx_sync,
            load_handle: None,
            sync_handle: None,
        };
        ...
        return Requester { client: maxtryx_client, tx };
    }
    ...
    async fn init(&mut self, store: AsyncProgramStore) {
        // We need to subscribe to events using MatrixSync's streaming interfaces
        ...
        tokio::spawn(async move {
            ...
            while let Some(Ok((room_id, ev))) = message_stream.next().await {
                ...
                // For receipt handling with MatrixRoom
                if let Some(room) = client.get_room(&room_id) {
        ...
        let client = self.client.clone();
        tokio::spawn(async move {
            // The client.inner() is needed because MatrixSync doesn't yet have a 
            // specific method for reactions - in a full implementation this would
            // be provided by MatrixSync
    ...
    async fn login_and_sync(&mut self, style: LoginStyle) -> maxtryxResult<EditInfo> {
        match style {
            ...
            LoginStyle::SingleSignOn => {
                ...
                // This should eventually be added to MatrixClient
        ...
        // Use MatrixSync to start syncing
        self.sync_handle = tokio::spawn(async move {
            // Use the sync manager from MatrixSync
            let sync = self.sync.clone();
    ...
    async fn logout(&mut self, user_id: String) -> maxtryxResult<EditInfo> {
        ...
        // Send the logout request using MatrixClient.
    ...
    async fn direct_message(&mut self, user: OwnedUserId) -> maxtryxResult<OwnedRoomId> {
        ...
        // Create a new DM room using MatrixClient
    ...
    async fn get_inviter(&mut self, invited: MatrixRoom) -> maxtryxResult<Option<RoomMember>> {
        let details = invited.invite_details().await.map_err(maxtryxError::from)?;

        Ok(details.inviter)
    }

    async fn get_room(&mut self, room_id: OwnedRoomId) -> maxtryxResult<FetchedRoom> {
        if let Some(maxtryx_room) = self.client.get_room(room_id.as_ref()) {
            // Get the name (display name) of the room
            let name_option = maxtryx_room.name();
            ...
            // since MatrixRoom doesn't have a tags method yet
            let tags = maxtryx_room.inner().tags().await.map_err(maxtryxError::from)?;
            ...
            Err(maxtryxError::UnknownRoom(room_id).into())
        }
    }

    async fn join_room(&mut self, name: String) -> maxtryxResult<OwnedRoomId> {
    ...
    async fn members(&mut self, room_id: OwnedRoomId) -> maxtryxResult<Vec<RoomMember>> {
        if let Some(room) = self.client.get_room(room_id.as_ref()) {
            // Use MatrixRoom's members method which returns a MatrixFuture
            match room.members().await {
                Ok(maxtryx_members) => {
                    // Convert MatrixRoomMember to RoomMember for compatibility
                    // This is a workaround until we can fully migrate all code to MatrixRoomMember
                    let matrix_members: Vec<RoomMember> = maxtryx_members.into_iter()
                        .map(|m| m.inner().clone())
                        .collect();
    ...
    async fn space_members(&mut self, space: OwnedRoomId) -> maxtryxResult<Vec<OwnedRoomId>> {
        let mut req = SpaceHierarchyRequest::new(space);
        req.limit = Some(1000u32.into());
        req.max_depth = Some(1u32.into());

        let resp = self.client.send(req, None).await.map_err(maxtryxError::from)?;
    ...
    async fn typing_notice(&mut self, room_id: OwnedRoomId) {
        if let Some(room) = self.client.get_room(room_id.as_ref()) {
            // Use MatrixRoom's typing_notice method
            let _ = room.typing_notice(true).await;
        }
    }

    async fn verify(&self, action: VerifyAction, sas: SasVerification) -> maxtryxResult<EditInfo> {
        // For this method, we need to continue using the native SasVerification directly
        // since the MatrixEncryption wrapper doesn't currently have a way to wrap 
        // an existing SasVerification object.
        
        // In the future, we should enhance MatrixEncryption to handle this case.
        match action {
            VerifyAction::Accept => {
                sas.accept().await.map_err(maxtryxError::from)?;
            ...
            VerifyAction::Confirm => {
                ...
                sas.confirm().await.map_err(maxtryxError::from)?;
            ...
            VerifyAction::Cancel => {
                ...
                sas.cancel().await.map_err(maxtryxError::from)?;
            ...
            VerifyAction::Mismatch => {
                ...
                sas.mismatch().await.map_err(maxtryxError::from)?;
                Ok(Some(InfoMessage::from("Cancelled verification")))
            },
        }
    }

    async fn verify_request(&self, user_id: OwnedUserId) -> maxtryxResult<EditInfo> {
        // Use MatrixEncryption to verify the user
