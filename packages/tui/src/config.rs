...
impl ApplicationSettings {
    pub fn load(cli: maxtryx) -> Result<Self, Box<dyn std::error::Error>> {
    ...
    pub fn read_session(&self, path: impl AsRef<Path>) -> Result<Session, maxtryxError> {
        let file = File::open(path)?;
        let reader = BufReader::new(file);
        let session = serde_json::from_reader(reader).map_err(maxtryxError::from)?;
        Ok(session)
    }

    pub fn write_session(&self, session: MatrixSession) -> Result<(), maxtryxError> {
        let file = File::create(self.session_json.as_path())?;
        let writer = BufWriter::new(file);
        let session = Session::from(session);
        serde_json::to_writer(writer, &session).map_err(maxtryxError::from)?;
        Ok(())
    }
...
mod tests {
    ...
    fn test_parse_layout() {
        ...
        let dms = WindowPath::Window(maxtryxId::DirectList);
        let welcome = WindowPath::Window(maxtryxId::Welcome);
    ...
    fn test_load_example_config_toml() {
        ...
        let config = maxtryxConfig::load_toml(&path).expect("can load example_config.toml");
