...
impl fmt::Display for MatrixAction {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            MatrixAction::Editor(action) => write!(f, "Editor({})", action),
            MatrixAction::Matrix(action) => write!(f, "Matrix({})", action),
        }
    }
}
...
pub mod matrix {
    ...
    pub fn switch_room(room_id: &str) -> MatrixAction {
        MatrixAction::Matrix(MatrixAction::SwitchRoom(room_id.to_string()))
    }

    pub fn send_message(msg: &str) -> MatrixAction {
        MatrixAction::Matrix(MatrixAction::SendMessage(msg.to_string()))
    }

    pub fn focus_chat_input() -> MatrixAction {
        MatrixAction::Matrix(MatrixAction::FocusChatInput)
    }

    pub fn focus_scrollback() -> MatrixAction {
        MatrixAction::Matrix(MatrixAction::FocusScrollback)
    }

    pub fn toggle_room_list() -> MatrixAction {
        MatrixAction::Matrix(MatrixAction::ToggleRoomList)
    }
}