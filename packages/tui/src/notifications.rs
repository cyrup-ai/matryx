...
use crate::{
    base::{AsyncProgramStore, maxtryxError, maxtryxResult, ProgramStore},
    config::{ApplicationSettings, NotifyVia},
};

const MATRYX_XDG_NAME: &str = match option_env!("MATRYX_XDG_NAME") {
    None => "maxtryx",
    Some(name) => name,
};
...
async fn send_notification(
    ...
    {
        let _ = (summary, body, MATRYX_XDG_NAME);
    }
...
fn send_notification_desktop(summary: &str, body: Option<&str>) {
    ...
    desktop_notification
        .summary(summary)
        .appname(MATRYX_XDG_NAME)
        .icon(MATRYX_XDG_NAME)
...
pub async fn parse_full_notification(
    ...
    room: MatrixRoom,
    show_body: bool,
) -> maxtryxResult<(String, Option<String>, MilliSecondsSinceUnixEpoch)> {
    let event = event.deserialize().map_err(maxtryxError::from)?;

    let server_ts = event.origin_server_ts();

    let sender_id = event.sender();
    let sender = room.get_member_no_sync(sender_id).await.map_err(maxtryxError::from)?;
    ...
    let body = if show_body {
        event_notification_body(
            &event,
            sender_name,
            room.is_direct().await.map_err(MatrixError::from)?,
        )
        .map(truncate)
    } else {
        None
...
fn truncate(s: String) -> String {
    ...
    if graphemes.clone().count() > MAX_LENGTH {
        let truncated: String = graphemes.take(MAX_LENGTH).collect();
        truncated + "..."
    } else {
        s
    }
