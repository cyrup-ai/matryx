//! # Default Keybindings
//!
//! The keybindings are set up here. We define some maxtryx-specific keybindings, but the default Vim
...
use crate::base::{maxtryxAction, maxtryxInfo, Keybindings, MATRIX_ID_WORD};
use crate::config::{ApplicationSettings, Keys};

pub type maxtryxStep = InputStep<maxtryxInfo>;
...
pub fn setup_keybindings() -> Keybindings {
    ...
    let stoggle = maxtryxStep::new()
        .actions(vec![maxtryxAction::ToggleScrollbackFocus.into()])
        .goto(VimMode::Normal);
    ...
    let newline = maxtryxStep::new().actions(vec![InsertTextAction::Type(
        Char::Single('\n').into(),
        MoveDir1D::Previous,
        1.into(),
    )
...
impl InputBindings<TerminalKey, maxtryxStep> for ApplicationSettings {
    fn setup(&self, bindings: &mut Keybindings) {
        for (modes, keys) in &self.macros {
            for (Keys(input, _), Keys(_, run)) in keys {
                ...
                let step = maxtryxStep::new().actions(vec![act.into()]);
