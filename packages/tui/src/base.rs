...
//!
//! The types defined here get used throughout maxtryx.
use std::borrow::Cow;
...
impl maxtryxAction {
    ...
    pub fn scribbles(&self) -> bool {
        matches!(self, maxtryxAction::Send(SendAction::SubmitFromEditor))
    }
}

impl From<HomeserverAction> for maxtryxAction {
    fn from(act: HomeserverAction) -> Self {
        maxtryxAction::Homeserver(act)
    }
}

impl From<MessageAction> for maxtryxAction {
    fn from(act: MessageAction) -> Self {
        maxtryxAction::Message(act)
    }
}

impl From<RoomAction> for maxtryxAction {
    fn from(act: RoomAction) -> Self {
        maxtryxAction::Room(act)
    }
}

impl From<SendAction> for maxtryxAction {
    fn from(act: SendAction) -> Self {
        maxtryxAction::Send(act)
    }
}

impl ApplicationAction for maxtryxAction {
    fn is_edit_sequence(&self, _: &EditContext) -> SequenceStatus {
        match self {
            maxtryxAction::ClearUnreads => SequenceStatus::Break,
            maxtryxAction::Homeserver(..) => SequenceStatus::Break,
            maxtryxAction::Keys(..) => SequenceStatus::Break,
            maxtryxAction::Message(..) => SequenceStatus::Break,
            maxtryxAction::Room(..) => SequenceStatus::Break,
            maxtryxAction::OpenLink(..) => SequenceStatus::Break,
            maxtryxAction::Send(..) => SequenceStatus::Break,
            maxtryxAction::ToggleScrollbackFocus => SequenceStatus::Break,
            maxtryxAction::Verify(..) => SequenceStatus::Break,
            maxtryxAction::VerifyRequest(..) => SequenceStatus::Break,
        }
    }

    fn is_last_action(&self, _: &EditContext) -> SequenceStatus {
        match self {
            maxtryxAction::ClearUnreads => SequenceStatus::Atom,
            maxtryxAction::Homeserver(..) => SequenceStatus::Atom,
            maxtryxAction::Keys(..) => SequenceStatus::Atom,
            maxtryxAction::Message(..) => SequenceStatus::Atom,
            maxtryxAction::OpenLink(..) => SequenceStatus::Atom,
            maxtryxAction::Room(..) => SequenceStatus::Atom,
            maxtryxAction::Send(..) => SequenceStatus::Atom,
            maxtryxAction::ToggleScrollbackFocus => SequenceStatus::Atom,
            maxtryxAction::Verify(..) => SequenceStatus::Atom,
            maxtryxAction::VerifyRequest(..) => SequenceStatus::Atom,
        }
    }

    fn is_last_selection(&self, _: &EditContext) -> SequenceStatus {
        match self {
            maxtryxAction::ClearUnreads => SequenceStatus::Ignore,
            maxtryxAction::Homeserver(..) => SequenceStatus::Ignore,
            maxtryxAction::Keys(..) => SequenceStatus::Ignore,
            maxtryxAction::Message(..) => SequenceStatus::Ignore,
            maxtryxAction::Room(..) => SequenceStatus::Ignore,
            maxtryxAction::OpenLink(..) => SequenceStatus::Ignore,
            maxtryxAction::Send(..) => SequenceStatus::Ignore,
            maxtryxAction::ToggleScrollbackFocus => SequenceStatus::Ignore,
            maxtryxAction::Verify(..) => SequenceStatus::Ignore,
            maxtryxAction::VerifyRequest(..) => SequenceStatus::Ignore,
        }
    }

    fn is_switchable(&self, _: &EditContext) -> bool {
        match self {
            maxtryxAction::ClearUnreads => false,
            maxtryxAction::Homeserver(..) => false,
            maxtryxAction::Message(..) => false,
            maxtryxAction::Room(..) => false,
            maxtryxAction::Keys(..) => false,
            maxtryxAction::Send(..) => false,
            maxtryxAction::OpenLink(..) => false,
            maxtryxAction::ToggleScrollbackFocus => false,
            maxtryxAction::Verify(..) => false,
            maxtryxAction::VerifyRequest(..) => false,
        }
    }
}
...
impl From<RoomAction> for ProgramAction {
    fn from(act: RoomAction) -> Self {
        maxtryxAction::from(act).into()
    }
}

impl From<maxtryxAction> for ProgramAction {
    fn from(act: maxtryxAction) -> Self {
        Action::Application(act)
    }
}

/// Alias for program actions.
pub type ProgramAction = Action<maxtryxInfo>;
/// Alias for program context.
pub type ProgramContext = EditContext;
/// Alias for program keybindings.
pub type Keybindings = VimMachine<TerminalKey, maxtryxInfo>;
/// Alias for a program command.
pub type ProgramCommand = VimCommand<maxtryxInfo>;
/// Alias for mapped program commands.
pub type ProgramCommands = VimCommandMachine<maxtryxInfo>;
/// Alias for program store.
pub type ProgramStore = Store<maxtryxInfo>;
/// Alias for shared program store.
pub type AsyncProgramStore = Arc<AsyncMutex<ProgramStore>>;
/// Alias for an action result.
pub type maxtryxResult<T> = UIResult<T, maxtryxInfo>;
...
impl From<maxtryxError> for UIError<maxtryxInfo> {
    fn from(err: maxtryxError) -> Self {
        UIError::Application(err)
    }
}

impl ApplicationError for maxtryxError {}
...
impl Display for maxtryxId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            maxtryxId::Room(room_id, None) => {
                write!(f, "maxtryx://room/{room_id}")
            },
            maxtryxId::Room(room_id, Some(thread)) => {
                write!(f, "maxtryx://room/{room_id}/threads/{thread}")
            },
            maxtryxId::MemberList(room_id) => {
                write!(f, "maxtryx://members/{room_id}")
            },
            maxtryxId::DirectList => f.write_str("maxtryx://dms"),
            maxtryxId::RoomList => f.write_str("maxtryx://rooms"),
            maxtryxId::SpaceList => f.write_str("maxtryx://spaces"),
            maxtryxId::VerifyList => f.write_str("maxtryx://verify"),
            maxtryxId::Welcome => f.write_str("maxtryx://welcome"),
            maxtryxId::ChatList => f.write_str("maxtryx://chats"),
            maxtryxId::UnreadList => f.write_str("maxtryx://unreads"),
        }
    }
}

impl ApplicationWindowId for maxtryxId {}

impl Serialize for maxtryxId {
...
impl<'de> Deserialize<'de> for maxtryxId {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        deserializer.deserialize_str(maxtryxIdVisitor)
    }
}

/// [serde] visitor for deserializing [maxtryxId].
struct maxtryxIdVisitor;

impl<'de> Visitor<'de> for maxtryxIdVisitor {
    type Value = maxtryxId;
        ...
        E: SerdeError,
    {
        let Ok(url) = Url::parse(value) else {
            return Err(E::custom("Invalid maxtryx window URL"));
        };

        if url.scheme() != "maxtryx" {
            return Err(E::custom("Invalid maxtryx window URL"));
        }
        ...
        match url.domain() {
            Some("room") => {
                ...
                match *path.collect::<Vec<_>>().as_slice() {
                    [room_id] => {
                        ...
                        Ok(maxtryxId::Room(room_id, None))
                    },
                    [room_id, "threads", thread_root] => {
                        ...
                        Ok(maxtryxId::Room(room_id, Some(thread_root)))
                    },
            ...
            Some("members") => {
                ...
                Ok(maxtryxId::MemberList(room_id))
            },
            Some("dms") => {
                if url.path() != "" {
                    return Err(E::custom("maxtryx://dms takes no path"));
                }

                Ok(maxtryxId::DirectList)
            },
            Some("rooms") => {
                if url.path() != "" {
                    return Err(E::custom("maxtryx://rooms takes no path"));
                }

                Ok(maxtryxId::RoomList)
            },
            Some("spaces") => {
                if url.path() != "" {
                    return Err(E::custom("maxtryx://spaces takes no path"));
                }

                Ok(maxtryxId::SpaceList)
            },
            Some("verify") => {
                if url.path() != "" {
                    return Err(E::custom("maxtryx://verify takes no path"));
                }

                Ok(maxtryxId::VerifyList)
            },
            Some("welcome") => {
                if url.path() != "" {
                    return Err(E::custom("maxtryx://welcome takes no path"));
                }

                Ok(maxtryxId::Welcome)
            },
            Some("chats") => {
                if url.path() != "" {
                    return Err(E::custom("maxtryx://chats takes no path"));
                }

                Ok(maxtryxId::ChatList)
            },
            Some("unreads") => {
                if url.path() != "" {
                    return Err(E::custom("maxtryx://unreads takes no path"));
                }

                Ok(maxtryxId::UnreadList)
            },
            Some(s) => Err(E::custom(format!("{s:?} is not a valid window"))),
            None => Err(E::custom("Invalid maxtryx window URL")),
        }
    }
}
...
impl maxtryxBufferId {
    /// Get the identifier for the window that contains this buffer.
    pub fn to_window(&self) -> Option<maxtryxId> {
        let id = match self {
            maxtryxBufferId::Command(_) => return None,
            maxtryxBufferId::Room(room, thread, _) => maxtryxId::Room(room.clone(), thread.clone()),
            maxtryxBufferId::DirectList => maxtryxId::DirectList,
            maxtryxBufferId::MemberList(room) => maxtryxId::MemberList(room.clone()),
            maxtryxBufferId::RoomList => maxtryxId::RoomList,
            maxtryxBufferId::SpaceList => maxtryxId::SpaceList,
            maxtryxBufferId::VerifyList => maxtryxId::VerifyList,
            maxtryxBufferId::Welcome => maxtryxId::Welcome,
            maxtryxBufferId::ChatList => maxtryxId::ChatList,
            maxtryxBufferId::UnreadList => maxtryxId::UnreadList,
        };
...
impl ApplicationContentId for maxtryxBufferId {}

impl ApplicationInfo for maxtryxInfo {
    type Error = maxtryxError;
    type Store = ChatStore;
    type Action = maxtryxAction;
    type WindowId = maxtryxId;
    type ContentId = maxtryxBufferId;

    fn complete(
        text: &EditRope,
        cursor: &mut Cursor,
        content: &maxtryxBufferId,
        store: &mut ProgramStore,
    ) -> Vec<String> {
        match content {
            maxtryxBufferId::Command(CommandType::Command) => complete_cmdbar(text, cursor, store),
            maxtryxBufferId::Command(CommandType::Search) => vec![],
            maxtryxBufferId::Room(_, _, RoomFocus::MessageBar) => complete_msgbar(text, cursor, store),
            maxtryxBufferId::Room(_, _, RoomFocus::Scrollback) => vec![],

            maxtryxBufferId::DirectList => vec![],
            maxtryxBufferId::MemberList(_) => vec![],
            maxtryxBufferId::RoomList => vec![],
            maxtryxBufferId::SpaceList => vec![],
            maxtryxBufferId::VerifyList => vec![],
            maxtryxBufferId::Welcome => vec![],
            maxtryxBufferId::ChatList => vec![],
            maxtryxBufferId::UnreadList => vec![],
        }
    }

    fn content_of_command(ct: CommandType) -> maxtryxBufferId {
        maxtryxBufferId::Command(ct)
    }
}
