...
//!
//! The window system is built around the `CyrumWindow` trait which provides a unified interface
...
use crate::modal::{
    Action, ActionResult, CyrumAction, DialogAction, EditAction, EditorAction, MovementAction,
...
impl CyrumWindow {
    /// Create a new window
    pub fn new(id: IambId, store: &mut ProgramStore) -> IambResult<Self> {
        match id {
    ...
    /// Get the window ID
    pub fn id(&self) -> IambId {
        match self {
            CyrumWindow::Room(room) => IambId::Room(room.id().to_owned(), room.thread().cloned()),
            CyrumWindow::DirectList(_) => IambId::DirectList,
            CyrumWindow::MemberList(_, room_id, _) => IambId::MemberList(room_id.clone()),
            CyrumWindow::RoomList(_) => IambId::RoomList,
            CyrumWindow::SpaceList(_) => IambId::SpaceList,
            CyrumWindow::VerifyList(_) => IambId::VerifyList,
            CyrumWindow::Welcome(_) => IambId::Welcome,
            CyrumWindow::ChatList(_) => IambId::ChatList,
            CyrumWindow::UnreadList(_) => IambId::UnreadList,
        }
    }
    ...
    pub fn tab_title(&self, store: &mut ProgramStore) -> Line {
        match self {
            CyrumWindow::DirectList(_) => bold_spans("Direct Messages"),
            CyrumWindow::RoomList(_) => bold_spans("Rooms"),
            CyrumWindow::SpaceList(_) => bold_spans("Spaces"),
            CyrumWindow::VerifyList(_) => bold_spans("Verifications"),
            CyrumWindow::Welcome(_) => bold_spans("Welcome to Cyrum"),
            CyrumWindow::ChatList(_) => bold_spans("DMs & Rooms"),
            CyrumWindow::UnreadList(_) => bold_spans("Unread Messages"),

            CyrumWindow::Room(w) => {
                let title = store.application.get_room_title(w.id());
                Line::from(title)
            },
            CyrumWindow::MemberList(state, room_id, _) => {
    ...
    pub fn window_title(&self, store: &mut ProgramStore) -> Line {
        match self {
            ...
            CyrumWindow::Room(w) => w.get_title(store),
            CyrumWindow::MemberList(state, room_id, _) => {
                let title = store.application.get_room_title(room_id.as_ref());
                let n = state.len();
                let v = vec![
    ...
    pub fn draw(&mut self, area: Rect, buf: &mut Buffer, focused: bool, store: &mut ProgramStore) {
        match self {
            CyrumWindow::Room(state) => state.draw(area, buf, focused, store),
            CyrumWindow::DirectList(state) => {
                self.draw_list(state, area, buf, focused, store, "Direct Messages", 
                    "No direct messages yet!");
            },
            CyrumWindow::MemberList(state, room_id, last_fetch) => {
            ...
            CyrumWindow::Welcome(state) => {
                state.draw(area, buf, focused, store);
            },
            CyrumWindow::ChatList(state) => {
    ...
    pub fn duplicate(&self, store: &mut ProgramStore) -> Self {
        match self {
            CyrumWindow::Room(w) => CyrumWindow::Room(w.dup(store)),
            CyrumWindow::DirectList(w) => CyrumWindow::DirectList(w.dup(store)),
            CyrumWindow::MemberList(w, room_id, last_fetch) => {
                CyrumWindow::MemberList(w.dup(store), room_id.clone(), *last_fetch)
            },
            CyrumWindow::RoomList(w) => CyrumWindow::RoomList(w.dup(store)),
            CyrumWindow::SpaceList(w) => CyrumWindow::SpaceList(w.dup(store)),
            CyrumWindow::VerifyList(w) => CyrumWindow::VerifyList(w.dup(store)),
            CyrumWindow::Welcome(w) => CyrumWindow::Welcome(w.dup(store)),
            CyrumWindow::ChatList(w) => CyrumWindow::ChatList(w.dup(store)),
            CyrumWindow::UnreadList(w) => CyrumWindow::UnreadList(w.dup(store)),
        }
    }
    ...
    pub fn close(&mut self, store: &mut ProgramStore) -> bool {
        match self {
            CyrumWindow::Room(w) => w.close(store),
            CyrumWindow::DirectList(w) => true, // Lists can always be closed
            CyrumWindow::MemberList(_, _, _) => true,
            CyrumWindow::RoomList(_) => true,
            CyrumWindow::SpaceList(_) => true,
            CyrumWindow::VerifyList(_) => true,
            CyrumWindow::Welcome(_) => true,
            CyrumWindow::ChatList(_) => true,
            CyrumWindow::UnreadList(_) => true,
        }
    }

    /// Toggle focus between components in a window
    pub fn focus_toggle(&mut self) {
        if let CyrumWindow::Room(w) = self {
            w.focus_toggle()
        }
    }
    ...
    pub fn execute_action(
        ...
        match self {
            CyrumWindow::Room(w) => w.editor_command(action, ctx, store),
            CyrumWindow::DirectList(w) => w.editor_command(action, ctx, store),
            CyrumWindow::MemberList(w, _, _) => w.editor_command(action, ctx, store),
            CyrumWindow::RoomList(w) => w.editor_command(action, ctx, store),
            CyrumWindow::SpaceList(w) => w.editor_command(action, ctx, store),
            CyrumWindow::VerifyList(w) => w.editor_command(action, ctx, store),
            CyrumWindow::Welcome(w) => w.editor_command(action, ctx, store),
            CyrumWindow::ChatList(w) => w.editor_command(action, ctx, store),
            CyrumWindow::UnreadList(w) => w.editor_command(action, ctx, store),
        }
    }
    ...
    pub fn jump(
        ...
        match self {
            CyrumWindow::Room(w) => w.jump(list, dir, count, ctx),
            CyrumWindow::DirectList(w) => w.jump(list, dir, count, ctx),
            CyrumWindow::MemberList(w, _, _) => w.jump(list, dir, count, ctx),
            CyrumWindow::RoomList(w) => w.jump(list, dir, count, ctx),
            CyrumWindow::SpaceList(w) => w.jump(list, dir, count, ctx),
            CyrumWindow::VerifyList(w) => w.jump(list, dir, count, ctx),
            CyrumWindow::Welcome(w) => w.jump(list, dir, count, ctx),
            CyrumWindow::ChatList(w) => w.jump(list, dir, count, ctx),
            CyrumWindow::UnreadList(w) => w.jump(list, dir, count, ctx),
        }
    }
    ...
    pub fn scroll(
        ...
        match self {
            CyrumWindow::Room(w) => w.scroll(style, ctx, store),
            CyrumWindow::DirectList(w) => w.scroll(style, ctx, store),
            CyrumWindow::MemberList(w, _, _) => w.scroll(style, ctx, store),
            CyrumWindow::RoomList(w) => w.scroll(style, ctx, store),
            CyrumWindow::SpaceList(w) => w.scroll(style, ctx, store),
            CyrumWindow::VerifyList(w) => w.scroll(style, ctx, store),
            CyrumWindow::Welcome(w) => w.scroll(style, ctx, store),
            CyrumWindow::ChatList(w) => w.scroll(style, ctx, store),
            CyrumWindow::UnreadList(w) => w.scroll(style, ctx, store),
        }
    }
    ...
    pub fn get_cursor_position(&self) -> Option<(u16, u16)> {
        match self {
            CyrumWindow::Room(w) => w.get_term_cursor().map(|tc| (tc.col, tc.row)),
            CyrumWindow::DirectList(w) => w.get_term_cursor().map(|tc| (tc.col, tc.row)),
            CyrumWindow::MemberList(w, _, _) => w.get_term_cursor().map(|tc| (tc.col, tc.row)),
            CyrumWindow::RoomList(w) => w.get_term_cursor().map(|tc| (tc.col, tc.row)),
            CyrumWindow::SpaceList(w) => w.get_term_cursor().map(|tc| (tc.col, tc.row)),
            CyrumWindow::VerifyList(w) => w.get_term_cursor().map(|tc| (tc.col, tc.row)),
            CyrumWindow::Welcome(w) => w.get_term_cursor().map(|tc| (tc.col, tc.row)),
            CyrumWindow::ChatList(w) => w.get_term_cursor().map(|tc| (tc.col, tc.row)),
            CyrumWindow::UnreadList(w) => w.get_term_cursor().map(|tc| (tc.col, tc.row)),
        }
    }
    ...
    pub fn get_completions(&self) -> Option<Vec<String>> {
        match self {
            CyrumWindow::Room(w) => w.get_completions().map(|cl| cl.items().collect()),
            CyrumWindow::DirectList(w) => w.get_completions().map(|cl| cl.items().collect()),
            CyrumWindow::MemberList(w, _, _) => w.get_completions().map(|cl| cl.items().collect()),
            CyrumWindow::RoomList(w) => w.get_completions().map(|cl| cl.items().collect()),
            CyrumWindow::SpaceList(w) => w.get_completions().map(|cl| cl.items().collect()),
            CyrumWindow::VerifyList(w) => w.get_completions().map(|cl| cl.items().collect()),
            CyrumWindow::Welcome(w) => w.get_completions().map(|cl| cl.items().collect()),
            CyrumWindow::ChatList(w) => w.get_completions().map(|cl| cl.items().collect()),
            CyrumWindow::UnreadList(w) => w.get_completions().map(|cl| cl.items().collect()),
        }
    }
    ...
    pub fn get_cursor_word(&self) -> Option<String> {
        match self {
            CyrumWindow::Room(w) => w.get_cursor_word(&crate::modal::WordStyle::Small),
            CyrumWindow::DirectList(w) => w.get_cursor_word(&crate::modal::WordStyle::Small),
            CyrumWindow::MemberList(w, _, _) => w.get_cursor_word(&crate::modal::WordStyle::Small),
            CyrumWindow::RoomList(w) => w.get_cursor_word(&crate::modal::WordStyle::Small),
            CyrumWindow::SpaceList(w) => w.get_cursor_word(&crate::modal::WordStyle::Small),
            CyrumWindow::VerifyList(w) => w.get_cursor_word(&crate::modal::WordStyle::Small),
            CyrumWindow::Welcome(w) => w.get_cursor_word(&crate::modal::WordStyle::Small),
            CyrumWindow::ChatList(w) => w.get_cursor_word(&crate::modal::WordStyle::Small),
            CyrumWindow::UnreadList(w) => w.get_cursor_word(&crate::modal::WordStyle::Small),
        }
    }
    ...
    pub fn get_selected_text(&self) -> Option<String> {
        match self {
            CyrumWindow::Room(w) => w.get_selected_word(),
            CyrumWindow::DirectList(w) => w.get_selected_word(),
            CyrumWindow::MemberList(w, _, _) => w.get_selected_word(),
            CyrumWindow::RoomList(w) => w.get_selected_word(),
            CyrumWindow::SpaceList(w) => w.get_selected_word(),
            CyrumWindow::VerifyList(w) => w.get_selected_word(),
            CyrumWindow::Welcome(w) => w.get_selected_word(),
            CyrumWindow::ChatList(w) => w.get_selected_word(),
            CyrumWindow::UnreadList(w) => w.get_selected_word(),
        }
    }
}
...
impl<T> ListState<T> {
    ...
    pub fn new(id: IambBufferId, items: Vec<T>) -> Self {
