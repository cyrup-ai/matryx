...
impl<T: Send + 'static> DaoSession<T> {
    ...
    /// Wait for multiple entity
    pub async fn entity(mut self) -> Result<Vec<T>> {
        match self.rx.recv().await {
            Some(DaoResponse::Entities(entity)) => Ok(entity),
            Some(DaoResponse::Error(err)) => Err(err),
            _ => Err(Error::other("Unexpected response type")),
        }
    }
...
/// Base DAO trait providing common CRUD operations for entity
    ...
    fn get_all(&self) -> DaoSession<T> {
        ...
        let handle = tokio::spawn(async move {
            match client.select::<T>(T::table_name()).await {
                Ok(entity) => {
                    let _ = tx.send(DaoResponse::Entities(entity)).await;
                }
                Err(err) => {
                    let _ = tx.send(DaoResponse::Error(err)).await;
                }
...
/// Common fields for database entity
