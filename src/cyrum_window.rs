...
use crate::modal::{
    ActionResult, CyrumAction, EditorAction, InputEvent, ModalState,
    WindowActionTrait,
};
...
/// Trait for windows in the Cyrum UI
pub trait CyrumWindow: WindowComponent {
    /// Get the window ID
    fn id(&self) -> &str;
    ...
    /// Execute a Cyrum-specific action
    fn execute_cyrum_action(&mut self, action: CyrumAction) -> ActionResult;
}
...
pub trait EditableWindow: CyrumWindow + WindowActionTrait {
...
/// Trait for matrix-specific window operations that integrates with CyrumClient wrappers
pub trait MatrixWindow: EditableWindow {
    /// Get the CyrumClient associated with this window
    fn client(&self) -> &cyrup_matrix::client::CyrumClient;
    
    /// Get the current room as a CyrumRoom if in a room view
    fn current_room(&self) -> Option<cyrup_matrix::room::CyrumRoom>;
...
/// Adapter trait to help converting between CyrumWindow and MatrixWindow traits.
...
pub trait MatrixWindowAdapter {
    ...
    fn get_client(&self) -> &CyrumClient;
    ...
    /// Transform a MatrixAction to CyrumAction for processing
    fn matrix_to_cyrum_action(&self, action: &MatrixAction) -> Option<CyrumAction>;
...
pub trait MatrixWindowDefault: MatrixWindow + MatrixWindowAdapter {
    ...
    fn default_handle_matrix_action(&self, action: &MatrixAction) -> ActionResult {
        // Convert MatrixAction to CyrumAction
        match self.matrix_to_cyrum_action(action) {
            Some(cyrum_action) => {
                // Execute the CyrumAction
                self.execute_cyrum_action(cyrum_action)
            }
            None => {
...
pub struct WindowRegistry {
    ...
    windows: std::collections::HashMap<String, Box<dyn CyrumWindow>>,
...
impl WindowRegistry {
    ...
    /// Register a window
    pub fn register<W: CyrumWindow + 'static>(&mut self, window: W) {
    ...
    /// Remove a window
    pub fn remove(&mut self, id: &str) -> Option<Box<dyn CyrumWindow>> {
    ...
    /// Get a window by ID
    pub fn get(&self, id: &str) -> Option<&dyn CyrumWindow> {
        self.windows.get(id).map(|w| w.as_ref())
    }
    
    /// Get a mutable window by ID
    pub fn get_mut(&mut self, id: &str) -> Option<&mut dyn CyrumWindow> {
    ...
    pub fn active_window(&self) -> Option<&dyn CyrumWindow> {
        self.active_window
            .as_ref()
    ...
    pub fn active_window_mut(&mut self) -> Option<&mut dyn CyrumWindow> {
