...
const CYRUM_XDG_NAME: &str = match option_env!("CYRUM_XDG_NAME") {
    None => "cyrum",
    Some(name) => name,
};
...
async fn send_notification(
    ...
    {
        let _ = (summary, body, CYRUM_XDG_NAME);
    }
...
pub async fn parse_full_notification(
    ...
    let body = if show_body {
        event_notification_body(
            &event,
            sender_name,
            room.is_direct().await.map_err(CyrumError::from)?,
        )
        .map(truncate)
    } else {
        None
