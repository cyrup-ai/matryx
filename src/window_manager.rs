//! Window management for the Cyrum application
//!
//! This module provides the main window management functionality for Cyrum,
...
use crate::{
    base::{IambId, IambResult, ProgramStore},
...
impl WindowManager {
    ...
    pub fn init(&mut self, store: &mut ProgramStore) -> IambResult<()> {
        let welcome = CyrumWindow::new(IambId::Welcome, store)?;
        self.add_window(welcome);
        Ok(())
    }

    /// Add a window to the manager
    pub fn add_window(&mut self, window: CyrumWindow) {
        self.windows.push(window);
        self.active_index = self.windows.len() - 1;
    }

    /// Get the active window
    pub fn active_window(&self) -> Option<&CyrumWindow> {
        self.windows.get(self.active_index)
    }

    /// Get the active window mutably
    pub fn active_window_mut(&mut self) -> Option<&mut CyrumWindow> {
    ...
    pub fn switch_to_window(&mut self, id: &IambId) -> bool {
    ...
    /// Open a new window
    pub fn open_window(&mut self, id: IambId, store: &mut ProgramStore) -> IambResult<()> {
        ...
        let window = CyrumWindow::new(id, store)?;
                ...
                InputEvent::Key(key) => {
                    ...
                    if let Some(action) = action {
                        match action {
                            ...
                            EditorAction::Cyrum(CyrumAction::ToggleScrollbackFocus) => {
                                window.focus_toggle();
                                true
                            }
            ...
            if self.modal_state.is_normal_mode() {
                match key.code {
                    ...
                    crossterm::event::KeyCode::Char('t') if key.has_ctrl() => {
                        ...
                        if let Ok(()) = self.open_window(IambId::Welcome, store) {
                            return true;
                        }
                    },
                    _ => {},
    ...
    fn handle_window_action(&mut self, action: &WindowAction, store: &mut ProgramStore) -> bool {
        match action {
            ...
            WindowAction::SwitchByName(name) => {
                ...
                if let Ok(window) = CyrumWindow::find(name.clone(), store) {
                    self.add_window(window);
                    true
                } else {
                    false
